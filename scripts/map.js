$(window).on("load", function () {
  ...
  function processAllPolygons() {
    ...
    while (
      ...
      if (divisors.length != polygonLayers.length) {
        alert(
          "Error in Polygons: The number of sets of divisors has to match the number of properties. Make sure that the cells in Polygon Properties, Property Ranges, and Property Popups all ahve the same number of divisor sets!"
        );
        return;
      }
  ...
  function onMapDataLoad(options, points, polylines) {
    ...
    $("#points-legend input+span").each(function (i) {
      // add to <span> that follows <input>
      ...
      $.getJSON(
        ...
        (function (index) {
          return function (data) {
            ...
            if (lines.filter(Boolean).length == p.length) {
              // only if all polylines loaded
              completePolylines = true;
